...
void perspective(float* result, float fovy, float aspect, float zNear, float zFar) {
...
static inline float rs(float number) {
	float x2 = number * 0.5f;
	float y = number;
	int32_t i = *(int32_t *)&y;
	i = 0x5f3759df - (i >> 1); // a86 for last three digits?
	y = *(float*) &i;
...
static inline vec3 dot(vec3 a, vec3 b) {
	return a.x * b.x + a.y * b.y + a.z * b.z;
}


static inline int mult(float* A, float* B) {
    float* C = 
