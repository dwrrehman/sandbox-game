...
x	- figure out how to render transluscent voxels. (for like, water, and gases, etc)
...
int main(void) {
	...
	for (int i = 0; i < 256; i++) pixels[i] = (uint8_t) (rand() % 256);         // random texture pixel data for now. 
	...
	glTexImage2D(
		GL_TEXTURE_2D, mip_level, internal_format, 
		width, height, border, format, type, pixels
	); 
	glGenerateMimap(GL_TEXTURE_2D);
