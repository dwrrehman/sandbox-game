...
static const nat side_length = 3000;
...
static uint32_t* get_at_position(nat home_x, nat home_y, int64_t x_off, int64_t y_off) {
	const int64_t s = side_length;
	int64_t x = (int64_t) home_x + x_off;
	int64_t y = (int64_t) home_y + y_off;
	x = (x + s) % s;
	y = (y + s) % s;
	return space + y * s + x;
}
...
static void generate_world(void) {
	...
	for (nat y = 0; y < s; y++) {
		for (nat x = 0; x < s; x++) {
			...
			space[s * y + x] = ((uint32_t) !(rand() % 16)) * dirt;
...
static void tick(void) {
	...
	for (nat y = 0; y < s; y++) {
		for (nat x = 0; x < s; x++) {
			...
			const uint32_t block = space[s * y + x];

			if (block == water and rand() % 5 == 0)  {

				int64_t r1 = rand() % 2;
				int64_t r2 = rand() % 2;

				if (*get_at_position(x, y, r1, r2) == air) {
					*get_at_position(x, y, r1, r2) = steam;

				} else if (*get_at_position(x, y, r1, r2) == steam) {
					*get_at_position(x, y, r1, r2) = air;
				}				
			}
		}
	}
