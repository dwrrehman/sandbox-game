...
int main(void) {
	...
	const float camera_accel = 0.09f;
	...
	while (not quit) {
		...
		qcheck(clEnqueueWriteBuffer(commands, space_array, CL_TRUE, 0, sizeof(nat) * (4 * unit_count + 1), space, 0, NULL, NULL));
...
// NEXT STEP IS:      implment octrees, or some wayyyy to decrease the amount of steps we do in order for a ray to hit a target. 

			// optimize to the case of     large areas being empty, because in reality, they will be. 
